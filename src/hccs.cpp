    ...
    for(int k=kbeg;k<kend;k++)
    {
        ...
        for(int j=jbeg;j<jend;j++)
        {
            ...
            {       
                ...
                double alpha0=coeff_weno7_c[0]*(1.0+std::pow(tau7/(ss+is0),2));
                double alpha1=coeff_weno7_c[1]*(1.0+std::pow(tau7/(ss+is1),2));
                double alpha2=coeff_weno7_c[2]*(1.0+std::pow(tau7/(ss+is2),2));
                double alpha3=coeff_weno7_c[3]*(1.0+std::pow(tau7/(ss+is3),2));
            ...
            for(int i=ibeg+1;i<(iend-1);i++)
            {
                ...
                double alpha0=coeff_crweno5_c[0]*(1.0+std::pow(tau/(is0+ss),2));
                double alpha1=coeff_crweno5_c[1]*(1.0+std::pow(tau/(is1+ss),2));
                double alpha2=coeff_crweno5_c[2]*(1.0+std::pow(tau/(is2+ss),2));
                ...
                {
                    a[i-ibeg]=(2.0/3.0*omega0+1.0/3.0*omega1)*(1.0-theta)/theta
                    +coeff_upwindcompact_c[0];

                    b[i-ibeg]=(1.0/3.0*omega0+2.0/3.0*(omega1+omega2))*(1.0-theta)/theta
                    +coeff_upwindcompact_c[1];

                    c[i-ibeg]=(1.0/3.0*omega2)*(1.0-theta)/theta
                    +coeff_upwindcompact_c[2];

                    d[i-ibeg]=(1.0/6.0*omega0*pData_in[i-1]
                               +(5.0*(omega0+omega1)+omega2)*pData_in[i]/6.0
                               +(omega1+5.0*omega2)*pData_in[i+1]/6.0)*(1.0-theta)/theta+q7;
                }
            }
    ...
    for(int k=kbeg;k<kend;k++)
    {
        ...
        for(int j=jbeg;j<jend;j++)
        {
            ...
            for(int i=ibeg+1;i<(iend-1);i++)
            {
                ...
                double is0= coeff_weno5_gamma0*std::pow((pData_in[i+3]-2.0*pData_in[i+2]+pData_in[i+1]),2)
                +coeff_weno5_gamma1*std::pow((pData_in[i+3]-4.0*pData_in[i+2]+3.0*pData_in[i+1]),2);
                
                double is1= coeff_weno5_gamma0*std::pow((pData_in[i+2]-2.0*pData_in[i+1]+pData_in[i]),2)
                +coeff_weno5_gamma1*std::pow((pData_in[i+2]-pData_in[i]),2);
                
                double is2= coeff_weno5_gamma0*std::pow((pData_in[i+1]-2.0*pData_in[i]+pData_in[i-1]),2)
                ...
                { 
                    c[i-ibeg]=(2.0/3.0*omega0+1.0/3.0*omega1)*(1.0-theta)/theta
                    +coeff_upwindcompact_c[0];
                    b[i-ibeg]=(1.0/3.0*omega0+2.0/3.0*(omega1+omega2))*(1.0-theta)/theta
                    +coeff_upwindcompact_c[1];
                    a[i-ibeg]=(1.0/3.0*omega2)*(1.0-theta)/theta
                    +coeff_upwindcompact_c[2];
                    d[i-ibeg]=(1.0/6.0*omega0*pData_in[i+2]
                               +(5.0*(omega0+omega1)+omega2)*pData_in[i+1]/6.0
                               +(omega1+5.0*omega2)*pData_in[i]/6.0)*(1.0-theta)/theta+q7;
                }
            }
